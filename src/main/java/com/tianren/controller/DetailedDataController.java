...
public class DetailedDataController {
    ...
    public String getRealTimeData(HttpServletRequest request, HttpSession session) {
        ...
        Result<Map<String, List<String>>> dataConfig = commonService.getDataConfig("detailedData");
    ...
    @ResponseBody
    @RequestMapping(value = "/appGetRealTimeData", produces = "application/json;charset=UTF-8")
    public String appGetRealTimeData(HttpServletRequest request, HttpSession session) {
        Result<List<SensorData>> sensorDataRes = commonService.getSensorDataByNum(START_ITEM_NUM, 1);
        return gson.toJson(sensorDataRes.getData().get(0));
    }

    @ResponseBody
    @RequestMapping(value = "/appStaticData", produces = "application/json;charset=UTF-8")
    public String appStaticData(HttpServletRequest request, HttpSession session) {
        Result<Map<String, Sensor>> sensorRes = commonService.getSensorResMap();
        return gson.toJson(sensorRes.getData());
    }
}

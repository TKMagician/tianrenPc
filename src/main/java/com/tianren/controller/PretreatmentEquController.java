...
@Controller
@RequestMapping("/pretreatmentEqu")
public class PretreatmentEquController {
    ...
    public String getRealTimeData(HttpServletRequest request, HttpSession session) {
        ...
        Result<Map<String, List<String>>> dataConfig = commonService.getDataConfig("pretreatmentEqu");
        ...
        return "pages/pretreatmentEqu";
    }

    @RequestMapping("/getHistoryData")
    public String getHistoryData(HttpServletRequest request, HttpSession session) {
        int startPage = Integer.parseInt(request.getParameter("startPage"));
        Result<List<SensorData>> sensorDataRes = commonService.getSensorDataByNum(0, 10);
        Result<Map<String, Sensor>> sensorRes = commonService.getSensorResMap();
        Result<Map<String, List<String>>> dataConfig = commonService.getDataConfig("pretreatmentEqu");
        if (!sensorDataRes.getResult() || !sensorRes.getResult()) {
            return null;//返回404--加在错误
